...
// here the challenge is dealing with relative paths and absolute paths
// check if it starts with a /

// check for relative path vs filename, do we treat them the same?
...
{
	int rv;
	rv = storage_readdir(path, buf, filler);
	printf("readdir(%s) -> %d\n", path, rv);
	return 0;
}
...
int
nufs_symlink(const char* file, const char* link) {

    int rv = 0;
    rv = storage_symlink(file, link);
...
int
nufs_readlink(const char* restrict path, char* restrict buf, size_t bufsize) 
{
    int rv = 0;
    rv = storage_readlink(path, buf, bufsize);
